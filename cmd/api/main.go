...
func main() {
	...
	router.Handle("/test-keys", middleware.RoleMiddleware(user_service, []string{"reading", "admin"}, http.HandlerFunc(test_keys_handler.GetTestKeys))).Methods("GET")
	...
	router.Handle("/test-keys/{id}", middleware.RoleMiddleware(user_service, []string{"reading", "admin"}, http.HandlerFunc(test_keys_handler.GetTestKeyByID))).Methods("GET")
